  ...
  return (
    ...
    <div>
      ...
      <div className="filter">
        ...
        <button onClick={() => setProperty({ text: "text", asc: null })}>default</button>
        <button onClick={() => setProperty({ text: "text", asc: true })}>a-z</button>
        <button onClick={() => setProperty({ text: "text", asc: false })}>z-a</button>
        <button onClick={() => setProperty({ price: "price", asc: true })}>inc</button>
        <button onClick={() => setProperty({ price: "price", asc: false })}>dec</button>
      </div>
          ...
          {data && data
            ...
            .sort((a, b) => {
              if (property && property.asc === true) {
                return (a[property.text] < b[property.text]) ? -1 : ((b[property.text] < a[property.text]) ? 1 : 0)
